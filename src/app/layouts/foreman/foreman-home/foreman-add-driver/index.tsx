...
export default function ForemanAddDriver() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (response?.data?.status) {
                showToast(t('driverAddedSuccess', 'Driver added successfully'));
                const driverData = response.data.data || {};

                // navigation.replace(STACKS.DRIVER_ADDED_SUCCESS as any, {
                //     driverName: fullName,
                //     tmId: driverData.user_id || driverData.tm_id || 'N/A',
                //     mobileNumber: `+91 ${mobileNumber}`,
                // });
            } else {
                showToast(response?.data?.message || t('failedToAddDriver', 'Failed to add driver'));
            }
        } catch (error: any) {
            console.log('Error adding driver:', error);
            showToast(error?.response?.data?.message || t('errorAddingDriver', 'Something went wrong'));
        } finally {
            setLoading(false);
        }
    };
