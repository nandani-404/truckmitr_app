...
const PunctureProfileEdit = () => {
    ...
    const [name, setName] = useState(user?.owner_name || user?.name || '');
    ...
    const profileImage = user?.profile_img || user?.image || user?.images || '';

    // Update state if user changes in redux
    useEffect(() => {
        if (user?.email) setEmail(user.email);
        if (user?.owner_name || user?.name) setName(user?.owner_name || user?.name || '');
    }, [user?.email, user?.name, user?.owner_name]);
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            formData.append('owner_name', name); // Puncture often uses owner_name
            ...
            console.log('DEBUG: Puncture Profile update formData:', formData);

            const response = await axiosInstance.post(END_POINTS.PUNCTURE_UPDATE_PROFILE, formData, {
                headers: { 'Content-Type': 'multipart/form-data' }
            });

            if (response.data && (response.data.status || response.data.success)) {
                console.log('DEBUG: Puncture Profile update success. Response:', response.data);
                showToast(t('profileUpdated', 'Profile updated successfully'));

                // Fetch fresh profile data to update Redux state correctly
                const profileRes = await axiosInstance.get(END_POINTS.GET_PROFILE);
                if (profileRes.data && profileRes.data.status) {
                    dispatch(userAction(profileRes.data));
                }

                console.log('DEBUG: Navigating to PUNCTURE_BOTTOM -> PUNCTURE_PROFILE');
                navigation.navigate(STACKS.PUNCTURE_BOTTOM as any, {
                    screen: STACKS.PUNCTURE_PROFILE
                });
            } else {
                console.log('DEBUG: Puncture Profile update failed. Response:', response.data);
                showToast(response.data?.message || t('updateFailed', 'Failed to update profile'));
            }

        } catch (error: any) {
            console.error('Puncture Profile update error:', error);
...
export default PunctureProfileEdit;
