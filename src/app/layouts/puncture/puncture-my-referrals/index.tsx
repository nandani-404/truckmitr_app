...
const StatusBadge = ({ status, t }: { status: string; t: any }) => {
    ...
    if (status === 'Paid') {
        ...
        translatedStatus = t('puncture_referrals_paid_status') || 'Paid';
    } else if (status === 'Pending') {
        bg = '#FEF3C7';
        color = '#D97706';
        icon = 'time';
        translatedStatus = t('puncture_referrals_pending');
    }

    return (
        <View style={[styles.badgeContainer, { backgroundColor: bg }]}>
            <Ionicons name={icon} size={12} color={color} style={{ marginRight: 4 }} />
            <Text style={[styles.badgeText, { color: color }]}>{translatedStatus}</Text>
        </View>
    );
};

export default function PunctureMyReferrals() {
    ...
    const renderItem = ({ item }: { item: any }) => {
        const getWalletStatusText = (walletStatus: string, amount: number) => {
            if (walletStatus === 'Credited') return `₹${amount} ${t('puncture_referrals_credited')}`;
            if (walletStatus === 'Pending') return t('puncture_referrals_pending');
            return t('puncture_referrals_not_eligible');
        };
            ...
            <View style={styles.card}>
                ...
                <View style={styles.cardFooter}>
                    <View style={styles.dateContainer}>
                        <Text style={styles.label}>{t('puncture_referrals_registered_on')}</Text>
                    ...
                    item.walletStatus === 'Credited' ? styles.walletSuccess :
                        ...
                        <Text style={[styles.walletText, {
                            ...
                            {getWalletStatusText(item.walletStatus, item.amount)}
                        </Text>
                    </View>
                </View>
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={[styles.header, { paddingTop: safeAreaInsets.top + 8 }]}>
                ...
                <Text style={styles.headerTitle}>{t('puncture_referrals_title')}</Text>
                <TouchableOpacity
            ...
            <View style={styles.statsContainer}>
                <View style={styles.statItem}>
                    ...
                    <Text style={styles.statLabel}>{t('puncture_referrals_total')}</Text>
                </View>
                <View style={styles.statDivider} />
                <View style={styles.statItem}>
                    <Text style={styles.statValue}>₹{totalEarned}</Text>
                    <Text style={styles.statLabel}>{t('puncture_referrals_total_earned')}</Text>
                </View>
                <View style={styles.statDivider} />
                <View style={styles.statItem}>
                    <Text style={styles.statValue}>{pendingCount}</Text>
                    <Text style={styles.statLabel}>{t('puncture_referrals_pending')}</Text>
                </View>
            </View>
            ...
            <View style={styles.filterRow}>
                <FlatList
                    ...
                    data={[
                        { key: 'All', label: t('puncture_referrals_all') },
                        { key: 'Paid', label: t('puncture_referrals_paid_status') || 'Paid' },
                        { key: 'Pending', label: t('puncture_referrals_pending') }
                    ]}
                ...
                <View style={styles.modalContainer}>
                    <View style={[styles.modalContent, { paddingBottom: safeAreaInsets.bottom + 20 }]}>
                        <View style={styles.modalHeader}>
                            <Text style={styles.modalTitle}>{t('puncture_referrals_filter_drivers')}</Text>
                        ...
                        <ScrollView style={styles.modalBody}>
                            ...
                            {/* Date Range Section */}
                            <Text style={styles.filterSectionTitle}>{t('puncture_referrals_date_range')}</Text>
                            <View style={styles.filterOptionsGrid}>
                                {[
                                    { key: 'All Time', label: t('puncture_referrals_all_time') },
                                    { key: 'Today', label: t('puncture_referrals_today') },
                                    { key: 'Last 7 Days', label: t('puncture_referrals_last_7_days') },
                                    { key: 'Last 30 Days', label: t('puncture_referrals_last_30_days') }
                                ].map((range) => (
                                    <TouchableOpacity
                                        key={range.key}
                                        style={[styles.optionChip, dateRange === range.key && styles.optionChipSelected]}
                                        onPress={() => setDateRange(range.key)}
                                    >
                                        <Text style={[styles.optionText, dateRange === range.key && styles.optionTextSelected]}>{range.label}</Text>
                                    </TouchableOpacity>
                                ))}
                            </View>
                            ...
                            {/* Status Section */}
                            <Text style={styles.filterSectionTitle}>{t('puncture_referrals_status')}</Text>
                            <View style={styles.filterOptionsGrid}>
                                {[
                                    { key: 'All', label: t('puncture_referrals_all') },
                                    { key: 'Paid', label: t('puncture_referrals_paid_status') || 'Paid' },
                                    ...
                                    <TouchableOpacity
                                        key={status.key}
                                        style={[styles.optionChip, statusFilter === status.key && styles.optionChipSelected]}
                                        onPress={() => setStatusFilter(status.key)}
                                    >
                                        <Text style={[styles.optionText, statusFilter === status.key && styles.optionTextSelected]}>{status.label}</Text>
                                    </TouchableOpacity>
                                ))}
                            </View>
                            ...
                            <View>
                                ...
                                >
                                    <Text style={[styles.dropdownHeaderText, { color: selectedStates.length > 0 ? '#111827' : '#6B7280' }]} numberOfLines={1}>
                                        {selectedStates.length > 0 ? selectedStates.join(', ') : t('puncture_referrals_select_state')}
                                    </Text>
                                        ...
                                        {ALL_STATES.map((state, index) => (
                                            ...
                                            >
                                                ...
                                                {selectedStates.includes(state) && (
                                                    <Ionicons name="checkmark" size={18} color="#475569" />
                                                )}
                                            </TouchableOpacity>
                                        ))}
                                    </View>
                        ...
                        <View style={styles.modalFooter}>
                            ...
                            >
                                <Text style={styles.resetBtnText}>{t('puncture_referrals_reset')}</Text>
                            </TouchableOpacity>
                            <TouchableOpacity
                                style={styles.applyBtn}
                                onPress={() => setIsFilterModalVisible(false)}
                            >
                                <Text style={styles.applyBtnText}>{t('puncture_referrals_apply_filters')}</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                </View>
...
const styles = StyleSheet.create({
    ...
    filterBadge: { width: 8, height: 8, borderRadius: 4, backgroundColor: '#475569', position: 'absolute', top: 6, right: 6 },
    ...
    optionChipSelected: { backgroundColor: '#F1F5F9', borderColor: '#475569' },
    optionText: { fontSize: 14, color: '#4B5563' },
    optionTextSelected: { color: '#475569', fontWeight: '600' },
    ...
    applyBtn: { flex: 1, padding: 14, borderRadius: 12, backgroundColor: '#475569', alignItems: 'center' },
