...
export default function ProfileCompletionForeman() {
    ...
    const submitProfile = async () => {
        ...
        try {
            ...
            if (userEdit?.driving_experience) formData.append('Driving_Experience', userEdit.driving_experience);
                ...
                return (
                    <View style={styles.stepContainer}>
                        ...
                        <TouchableOpacity onPress={() => setYearPickerOpen(true)} style={styles.classicBox}>
                            <Text style={{ color: userEdit?.DOB ? '#333' : '#999' }}>{userEdit?.DOB ? moment(userEdit.DOB).format('DD MMM YYYY') : 'Select DOB'}</Text>
                        ...
                        <Space height={20} />
                        <Text style={styles.classicLabel}>{t('yearsOfExperience') || "Years of Experience"}</Text>
                        <View style={styles.experienceGrid}>
                            {['< 1 Year', '1-2 Years', '3-5 Years', '6-10 Years', '10+ Years'].map((exp) => {
                                const isSelected = userEdit?.driving_experience === exp;
                                return (
                                    <TouchableOpacity
                                        key={exp}
                                        style={[styles.experienceCard, isSelected && styles.experienceCardSelected]}
                                        onPress={() => dispatch(userEditAction({ ...userEdit, driving_experience: exp }))}
                                    >
                    ...
                    <View style={styles.stepContainer}>
                        ...
                        <Modal visible={licenseExpiryModal} transparent animationType="fade">
                            <View style={styles.yearPickerOverlay}>
                                <View style={[styles.yearPickerContainer, { alignItems: 'center' }]}>
                                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, width: '100%' }}>
                                        <Text style={{ fontSize: 18, fontWeight: '600', color: '#333' }}>{t('selectExpiryDate') || 'Select Expiry Date'}</Text>
            ...
            case 'work_details':
                return (
                    <View style={styles.stepContainer}>
                        ...
                        <TextInput
                            style={styles.classicInput}
                            placeholder="e.g. North India, Mumbai-Delhi"
