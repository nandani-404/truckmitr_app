...
interface VerifiedDriversResponse {
    success: boolean;
    ...
    const handleViewDetails = () => {
        ...
        navigation.navigate(STACKS.DRIVER_ASSOCIATION_DRIVER_DETAILS, {
            driver: {
                ...
                amount: 0, // Placeholder or fetch if available
            ...
            <View style={styles.detailsContainer}>
                ...
                <View style={styles.detailRow}>
                    ...
                    <View style={styles.detailItem}>
                        ...
                        <Text style={[styles.detailText, { color: '#D97706', fontWeight: '600' }]}>{driver.remaining_days} days left</Text>
                    </View>
                </View>
            </View>
...
export default function DriverAssociationVerifiedDrivers() {
    ...
    const fetchVerifiedDrivers = useCallback(async (isRefreshing = false) => {
        if (!user?.id) return;

        try {
            if (!isRefreshing) setLoading(true);
            const response = await axiosInstance.get(END_POINTS.GET_ASSOCIATION_VERIFIED_DRIVER(user.id));
            if (response.data) {
                setDrivers(response.data.drivers || []);
            }
        } catch (error) {
            console.error('Error fetching verified drivers:', error);
            Alert.alert('Error', 'Failed to fetch verified drivers. Please try again.');
        } finally {
            setLoading(false);
            setRefreshing(false);
        }
