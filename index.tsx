...
const ClearIcon = ({ size = 16, color = "#9ca3af" }: { size?: number, color?: string }) => (
...
const ShipperPostLoad = () => {
    ...
    const fetchVehicleTypesByQuantity = async (qty: string) => {
        ...
        try {
            ...
            // Dummy fallback if API fails or is unavailable
            setVehicleBodies([
                { id: 1, name: 'Open Body', icon: '🚛' },
                { id: 2, name: 'Container', icon: '📦' },
                { id: 3, name: 'Trailer', icon: '🚚' },
            ]);
        } finally {
    ...
    const fetchVehicleLengths = async (bodyId: number, qty: string) => {
        ...
        try {
            ...
            setVehicleLengths([{ id: 1, length_label: '20 ft' }, { id: 2, length_label: '32 ft SXL' }]);
        } finally {
    ...
    const handleNext = () => {
        ...
        // Material Type is technically optional if 'Select Material' is default, but let's enforce?
        // Actually user said 'step 6 show material type'.
        if (step.id === 'material_type') {
            // Keep it optional or dummy select
        }
    ...
    const handleSubmit = async () => {
        ...
        try {
            const payload = {
                ...
                meterial: materialId || 1, // Fallback
                                    ...
                                    {vehicleBodies.map((body, index) => (
                                        <TouchableOpacity
                                            ...
                                            onPress={() => {
                                                ...
                                                setSelectedVehicleId(null); // Reset length
                                            }}
                                        >
            ...
            case 'material_type':
                return (
                    <View style={styles.stepContainer}>
                        ...
                        {/* Simple Material Modal Logic inline or external? Keeping it simple for now, using a temporary list */}
                        {isMaterialVisible && (
                            <View style={styles.suggestionOverlay}>
                                {['Electronics', 'Agriculture', 'Machinery', 'Textiles', 'Chemicals', 'Others'].map((m) => (
                                    <TouchableOpacity key={m} style={styles.suggestionItem} onPress={() => {
                                        setMaterialType(m);
                                        // Mock ID or fetch real one
                                        // setMaterialId(...)
                                ...
                                <TouchableOpacity style={styles.suggestionItem} onPress={() => setMaterialVisible(false)}>
    ...
    return (
        <View style={{ flex: 1, backgroundColor: '#FFFFFF' }}>
            ...
            <KeyboardAwareScrollView
                ...
                scrollEnabled={!((originSuggestions.length > 0 && !isSearchingOrigin) || (destSuggestions.length > 0 && !isSearchingDest) || isMaterialVisible)}
            >
...
const styles = StyleSheet.create({
    ...
    scrollContent: {
        ...
        paddingTop: 24,
        paddingBottom: 120, // Extra padding to allow scrolling above keyboard
    },
    stepHeader: {
        flexDirection: 'row',
        alignItems: 'center',
    ...
    footer: {
        paddingHorizontal: 24,
        paddingTop: 16,
        paddingBottom: 30, // Safe area hint handles in component
        borderTopWidth: 1,
